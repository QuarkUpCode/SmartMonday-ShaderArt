...
float sdf_square(float2 p, float2 center, float len){
	
	float2 d = (float2){fabs(p.x-center.x), fabs(p.y-center.y)};

	if(max(d.x, d.y) < len) return max(d.x, d.y) - len;

	if(d.x < len) return d.y-len;
	if(d.y < len) return d.x-len;

	return length(d - (float2){len, len});

}

float4 sdf(float2 p, float4 colorA, float4 colorB, float4 colorC, float t){

	const float4 BLACK = {0.f, 0.f, 0.f, 1.f};
	...
	float v = INFINITY;
	v = min(v, sdf_square(p, origin, 0.1f));

	if(fabs(v) < outline){
		return WHITE;
	}
	float f = 0.5f + 0.5f*sin(fabs(v)*2.f*M_PI_F*F + (phi*t));
	if(f > 0.75f) return colorA;
	return colorB;	
}
