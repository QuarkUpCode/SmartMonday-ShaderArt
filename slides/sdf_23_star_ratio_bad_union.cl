...
float sdf_persona_star(float2 p, float2 center, float n, float dtheta, float radius){
...
float4 sdf(float2 p, float4 colorA, float4 colorB, float4 colorC, float t){
	...
	float v = INFINITY;
	v = min(v, sdf_persona_star(p, origin, 5.f, 0.f, 0.2f));
	v = min(v, sdf_persona_star(p, (float2){0.1f, 0.1f}, 5.f, 0.f, 0.2f));
